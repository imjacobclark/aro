...
struct PlaybackOutputStatus: Equatable, Sendable {
    var mode: PlaybackMode = .bitPerfect
    ...
    var appliedGainDecibels: Double = 0
    var warning: String?
...
enum PlaybackEngineError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .outputDeviceMismatch(let requested, let actual):
            return "Core Audio could not route playback to \(requested). The active output is \(actual)."
...
final class HighResolutionPlaybackEngine: NSObject, AudioPlaybackEngine {
    ...
    func load(
        ...
        if let device {
            ...
            let acquiredHog = try deviceManager.configure(
                ...
                acquireHogMode: preferences.hogModeEnabled
            )
        ...
        outputStatus.mode = preferences.mode
        ...
        configureProcessingGraph(for: preferences.mode)
        installMeterTap()
    ...
    private func compatibleRun(
        ...
        for candidateIndex in index..<songs.count {
            ...
            if preferences.mode == .normalized, song.loudness == nil {
                break
            }
...
private final class AudioMeterRelay: @unchecked Sendable {
...
nonisolated private func makeAudioMeterTapBlock(
...
private func meterLevels(
    ...
    return (0..<barCount).map { bar in
        ...
        // outer radius for strong transients. A −36 dB floor removes visual
        // noise, while the eased curve still lets peaks near −3 dB fully bloom.
