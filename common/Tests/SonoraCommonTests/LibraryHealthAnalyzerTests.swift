...
@Suite("SQLite library health integration")
struct LibraryDatabaseHealthIntegrationTests {
    @Test("Database rows are mapped into domain analysis")
    func databaseDelegatesToAnalyzer() throws {
        ...
        _ = database.reconcile(
            songs: [
                song(
                    at: directory.appendingPathComponent("A.flac"),
                    contentHash: "same-content",
                    fileSize: 1_000
                ),
                song(
                    at: directory.appendingPathComponent("B.flac"),
                    contentHash: "same-content",
                    fileSize: 2_000
                )
            ],
        ...
        let recommendation = try #require(
            useCase.execute().exactDuplicates.first
        )
        #expect(recommendation.copies.count == 2)
        #expect(recommendation.potentialSavingsBytes == 2_000)
    }
...
@Suite("Library health persistence mapping")
struct LibraryHealthTrackMapperTests {
    @Test("Invalid persistence identities never enter the domain")
    func rejectsInvalidIdentity() {
        ...
        #expect(LibraryHealthTrackMapper().map(record) == nil)
    }
}
#endif
