...
final class LibraryDatabaseTests: XCTestCase {
    ...
    func testLoudnessAndHiddenStateSurviveReopeningDatabase() throws {
        ...
        reopened.setHidden(trackID: storedSong.libraryID, hidden: true)
    ...
    func testLibraryAndListeningStatsArePersisted() throws {
        ...
        let sessionID = database.beginListeningSession(
            trackID: track.libraryID
        )
        Thread.sleep(forTimeInterval: 0.02)
        database.endListeningSession(id: sessionID, completed: true)

        let reopened = LibraryDatabase(url: databaseURL)
        let library = reopened.libraryStats()
        ...
        let listening = reopened.listeningStats()
