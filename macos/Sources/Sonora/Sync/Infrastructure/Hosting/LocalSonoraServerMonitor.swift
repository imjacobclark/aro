...
enum LocalSonoraServerKind: String, Hashable, Sendable {
    case bundledHelper
    case standalone
...
struct LocalSonoraServer: Identifiable, Hashable, Sendable {
    let pid: Int32
    let kind: LocalSonoraServerKind
    let listener: String
    let hubID: UUID?
...
@MainActor
@Observable
final class LocalSonoraServerMonitor {
    ...
    func stopStandalone(_ server: LocalSonoraServer) {
    ...
    nonisolated static func configurationPath(
        ...
        return homeDirectory
            .appendingPathComponent(
                "Library/Application Support/Sonora/Server/sonora.toml"
            )
            .path
    }
    ...
    private func makeServer(
        ...
        let kind: LocalSonoraServerKind = listener.pid == helperPID
            ? .bundledHelper
            : .standalone
        ...
        return LocalSonoraServer(
            pid: listener.pid,
            kind: kind,
            listener: listener.address,
            hubID: config["hub_id"].flatMap(UUID.init(uuidString:)),
            displayName: config["display_name"] ?? "Sonora Server",
            dataPath: dataPath,
...
private enum LocalServerMonitorError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .commandFailed(let output):
            output.isEmpty
                ? "Unable to inspect local Sonora servers."
                : output
        }
    }
}
