...
actor SonoraLibraryExporter {
    private let client: SonoraSyncClient
    private let credential: HubDeviceCredential?
    private let fileManager = FileManager.default

    init(client: SonoraSyncClient, credential: HubDeviceCredential?) {
        self.client = client
    ...
    func export(
        ...
        try encoder.encode(manifest).write(
            to: root.appendingPathComponent("sonora-manifest.json"),
            options: .atomic
        )
        await progress(
            LibraryExportProgress(
    ...
    private func download(
        _ track: SonoraExportTrack,
        to target: URL
    ) async throws {
        let partial = target.appendingPathExtension("sonora-part")
