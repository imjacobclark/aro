...
struct LibrarySetupView: View {
    ...
    private var choice: some View {
        VStack(alignment: .leading, spacing: 18) {
            ...
            setupCard(
                ...
                detail: "Keep your music on this Mac and make it available to your other Sonora devices.",
            ...
            setupCard(
                ...
                detail: "Use a Sonora library already running elsewhere.",
    ...
    private var storage: some View {
        VStack(alignment: .leading, spacing: 20) {
            Text("Choose how Sonora stores your music")
                .font(.title2)
            selectionCard(
                selected: storageChoice == .stored,
                title: "Stored by Sonora",
                detail: "Recommended. Sonora keeps its own bit-identical copy, so the library remains available if an original folder or contributing device goes offline."
            ) {
            ...
            selectionCard(
                ...
                title: "Linked files",
                detail: "Sonora reads music from its current location without making another copy. Linked libraries cannot accept uploads and unavailable folders cannot be served."
            ) {
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            Toggle(isOn: $sharingEnabled) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Allow my other Sonora devices to connect")
                        .font(.headline)
                    Text(
                        "Sonora makes this library available on your local network. Only devices you approve can connect."
                    )
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                }
    ...
    private var managedLibraryURL: URL {
        FileManager.default.urls(
            for: .musicDirectory,
            in: .userDomainMask
        )[0].appendingPathComponent("Sonora Music Library", isDirectory: true)
    }
}
