...
final class FolderMonitor: FolderMonitoring, @unchecked Sendable {
    ...
    init?(url: URL, handler: @escaping @Sendable () -> Void) {
        ...
        FSEventStreamSetDispatchQueue(
            stream,
            DispatchQueue(label: "com.aro.folder-monitor.\(UUID().uuidString)")
        )
...
struct FSEventsFolderMonitorFactory: FolderMonitorCreating {
    func makeMonitor(
        for url: URL,
        handler: @escaping @Sendable () -> Void
    ) -> (any FolderMonitoring)? {
        FolderMonitor(url: url, handler: handler)
    }
}
