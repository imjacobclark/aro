...
struct LoudnessAnalysisServiceTests {
    ...
    func skipsRemoteMedia() async {
        let service = LoudnessAnalysisService(
            cacheURL: URL(fileURLWithPath: "/private/tmp/sonora-missing-loudness-cache.json"),
    ...
    func skipsMissingLocalMedia() async {
        ...
        let song = Song(
            url: URL(fileURLWithPath: "/private/tmp/sonora-file-that-does-not-exist.flac"),
            title: "Missing song",
            artist: "Artist",
            duration: 180,
            fileFingerprint: AudioFileFingerprint(
                standardizedPath: "/private/tmp/sonora-file-that-does-not-exist.flac",
                fileSizeBytes: 1_024,
                modificationDate: .distantPast,
                contentHash: "missing"
            )
